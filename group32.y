%token IF ELSE DRONE PROGRAM_BEGIN PROGRAM_END TIMER TYPE_STRING TYPE_FLOAT TYPE_INT TYPE_BOOL CONST RETURN TRUE FALSE WHILE_LOOP FOR_LOOP OR AND NOT PRINT_STATEMENT VOID_FUNC WIFI_IDENTIFIER INCLINE_FUNC ALTITUDE_FUNC ACCEL_FUNC TEMPR_FUNC ON_FUNC OFF_FUNC PICTURE_FUNC CAPTURE_FUNC CONNECT_FUNC MAIN_FUNC TIME_FUNC TIMER_FUNC FOR_LOOP VARIABLE INTEGER COMMA BELONGS SMALLER_THAN BIGGER_THAN ADD SUBTRACT DIVIDE MULTIPLY LP RP LC RC LSQ  END_STATEMENT RSQ BIGGER_THAN LESS_THAN BIGGER_EQUAL LESS_EQUAL EQUAL NOT_EQUAL  SEMI_COL ASGN_OP COMMENT QUOTE 
%right '='
%left '+' '-'
%left '#''|'
%%
